 ...
 * グレイスケールに変換
 */
JNIEXPORT void JNICALL Java_com_vsn_edit_cv_FilterImage_convertToGrayScale(
		JNIEnv* env, jobject thiz) {
	LOGI("GrayScale Start");
	// 符号なし8bit、ピクセル毎のチャンネル数1のインタリーブ画像の作成　作業領域確保
	IplImage *pWorkImage = cvCreateImage(cvGetSize(pImage),IPL_DEPTH_8U,1);	//　グレイスケールに変換した画像を格納する作業領域
	// cvCvtColor : 画像の色空間を変換する.BGR形式からグレースケール変換
	cvCvtColor(pImage,pWorkImage,CV_BGR2GRAY);	//　pImageを1チャンネルのグレースケール変換したものをpWorkImageに出力
	cvCvtColor(pWorkImage,pImage,CV_GRAY2BGR);	//　1チャンネルのグレースケール変換したpWorkImageを3チャンネルのBGR形式に変換してpImageに出力
	// cvReleaseImage : ヘッダと画像データを解放する
	cvReleaseImage(&pWorkImage);		// 作業領域の開放
	LOGI("GrayScale Done.");
}
 ...
 * ぼかしをかける
 */
JNIEXPORT void JNICALL Java_com_vsn_edit_cv_FilterImage_convertToBokashi(
		JNIEnv* env, jobject thiz) {
	cvSmooth(pImage, pImage, CV_BLUR, 5, 0, 0, 0);	// BLURで画像の平滑化をする  BLUR:画像がぶれたような
//	cvSmooth(pImage, pWorkImage, CV_GAUSSIAN, 5, 0, 0, 0);	// 各画素に重みをつけて平滑化する、注目画素に近いほど重みが大きい
//	cvSmooth(pImage, pWorkImage, CV_MEDIAN, 5, 0, 0, 0);	// 周辺の画素の中央値で平滑化する
}
 ...
 * 色の反転処理をかける
 */
JNIEXPORT void JNICALL Java_com_vsn_edit_cv_FilterImage_convertToReverse(
		JNIEnv* env, jobject thiz) {
	// cvNot : Bitの反転
	cvNot(pImage, pImage);		// 色情報のビットを反転する
}
 ...
 * モザイクをかける
 */
JNIEXPORT void JNICALL Java_com_vsn_edit_cv_FilterImage_convertToMosaic(
		JNIEnv* env, jobject thiz) {
	// 符号なし8bit、ピクセル毎のチャンネル数3のインタリーブ画像の作成　作業領域確保
	IplImage *pSmallWorkImage = cvCreateImage(cvSize((int)(pImage->width*0.05+0.5),(int)(pImage->height*0.05+0.5)),IPL_DEPTH_8U,3);		//　縮小用の作業領域

	cvResize(pImage,pSmallWorkImage,CV_INTER_AREA);// pImageの画像をピクセル領域の関係を用いたリサンプリングで縮小 したものをpSmallWorkImageに格納.
	cvResize(pSmallWorkImage,pImage,CV_INTER_NN);	//　縮小した画像を元の画像のサイズに最近隣接補間で拡大をし解像度を下げたものを、pImageに格納.

	// cvReleaseImage : ヘッダと画像データを解放する
	cvReleaseImage(&pSmallWorkImage);		// 作業領域の開放
}
 ...
 * jint height : 高さ
 */
JNIEXPORT jbyte JNICALL Java_com_vsn_edit_cv_FilterImage_setSourceImage(
 ...
 */
JNIEXPORT jbyteArray JNICALL Java_com_vsn_edit_cv_FilterImage_getSourceImage(
		JNIEnv* env, jobject thiz) {
	if (pImage == NULL) {
